...
bool HTU31D::begin(uint8_t i2c_addr, TwoWire *theWire) {
  ...
  i2c_dev = new I2CDevice(i2c_addr, theWire);
  ...
  humidity_sensor = new HTU31D_Humidity(this);
  temp_sensor = new HTU31D_Temp(this);
 ...
 */
uint32_t HTU31D::readSerial(void) {
  uint8_t reply[4];
  uint32_t serial = 0;

  BusIO_Register sernumreg =
      BusIO_Register(i2c_dev, HTU31D_READSERIAL, 4);
...
*/
/**************************************************************************/
bool HTU31D::getEvent(sensors_event_t *humevent,
  ...
  BusIO_Register threg =
      BusIO_Register(i2c_dev, HTU31D_READTEMPHUM, 6);
  uint8_t thdata[6];
...
void HTU31D::fillTempEvent(sensors_event_t *temp, uint32_t timestamp) {
...
void HTU31D::fillHumidityEvent(sensors_event_t *humidity,
                                        uint32_t timestamp) {
...
/**
 * @brief Gets the Sensor object for the HTU31D's humidity sensor
 *
 * @return Sensor*
 */
Sensor *HTU31D::getHumiditySensor(void) {
 ...
 * @return Sensor*
 */
Sensor *HTU31D::getTemperatureSensor(void) {
  return temp_sensor;
}
/**
 ...
 */
void HTU31D_Humidity::getSensor(sensor_t *sensor) {
...
/**
    ...
    @returns True
 */
bool HTU31D_Humidity::getEvent(sensors_event_t *event) {
 ...
 * @param sensor The sensor_t object to be populated
 */
void HTU31D_Temp::getSensor(sensor_t *sensor) {
  /* Clear the sensor_t object */
  memset(sensor, 0, sizeof(sensor_t));
...
/*!
    ...
    @returns true
*/
bool HTU31D_Temp::getEvent(sensors_event_t *event) {
